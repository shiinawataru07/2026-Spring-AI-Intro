...
class NaiveBayesModel:
    def __init__(self):
        ...
        self.wordsCnt = [0, 0] # 正负样本中所有token出现总次数
        self.count()
    ...
    def count(self):
        ...
        for i in range(self.dataset.len):
            ...
            self.wordsCnt[label] += len(text)
            for word in text:
                self.token_num[label][word] = self.token_num[label].get(word, 0) + 1
                voc.add(word)
        self.V = len(voc)
    ...
    def __call__(self, text):
        # TODO: YOUR CODE HERE
        # 返回1或0代表当前句子分类为正/负样本
        logPPos = math.log(self.pos_neg_num[1] / self.dataset.len)
        logPNeg = math.log(self.pos_neg_num[0] / self.dataset.len)
        alpha = 1
        for word in text:
            logPPos += math.log((self.token_num[1].get(word, 0) + alpha) / (self.wordsCnt[1] + alpha * self.V))
            logPNeg += math.log((self.token_num[0].get(word, 0) + alpha) / (self.wordsCnt[0] + alpha * self.V))
        return 1 if logPPos > logPNeg else 0
...
class Embedding():
    ...
    def __call__(self, text, max_len=50):
        ...
        ret = np.zeros((max_len, 100))
        index = 0
        for word in text:
            if word in self.emb:
                ret[index] = self.emb[word]
                index += 1
            if index >= max_len:
                break
        return ret
...
class QAModel():
    ...
    def idf(self, word):
        ...
        for doc in self.document_list:
            if word in doc['document']:
                d += 1
        return np.log10(D / d) if d > 0 else 0
    ...
    def __call__(self, query):
        ...
        for index in range(n):
            ...
            if score > bestScore:
                bestScore = score
                bestDocumentIndex = index
        bestDocument = self.document_list[bestDocumentIndex]
        bestScore = -1
        bestSentence = ""
        for sentence in bestDocument['sentences']:
            score = 0
            for word in query:
