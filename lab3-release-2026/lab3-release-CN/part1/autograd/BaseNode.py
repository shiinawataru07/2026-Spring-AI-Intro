...
class Attention(Node):
    ...
    def cal(self, X):
        ...
        K = X @ W_k
        V = X @ W_v
        S = np.einsum("...id,...jd->...ij", Q, K) / np.sqrt(self.dim)
        S = S - np.max(S, axis=-1, keepdims=True)
        softmaxScore = np.exp(S)
        softmaxScore = softmaxScore / np.sum(softmaxScore, axis=-1, keepdims=True)
        O = softmaxScore @ V
...
class Linear(Node):
    ...
    def backcal(self, grad):
        ...
        weightGrad = X.T @ grad
        biasGrad = np.sum(grad, axis=0)
        self.grad.append(weightGrad)
        self.grad.append(biasGrad)
...
class ResLinear(Node):
    ...
    def backcal(self, grad):
        ...
        X = self.cache[-1]
        weight = self.params[0]
        weightGrad = X.transpose(0, 2, 1) @ grad
        weightGrad = np.sum(weightGrad, axis=0)
        biasGrad = np.sum(grad, axis=0)
        biasGrad = np.sum(biasGrad, axis=0)
        self.grad.append(weightGrad)
        self.grad.append(biasGrad)
...
class LogSoftmax(Node):
    ...
    def cal(self, X):
        ...
        expX = np.exp(X)
        sumExpX = np.sum(expX, axis=self.dim, keepdims=True)
        logsoftmaxX = X - np.log(sumExpX)
        self.cache.append(logsoftmaxX)
        return logsoftmaxX
    ...
    def backcal(self, grad):
        ...
        return grad - np.multiply(softmaxX, np.sum(grad, axis=self.dim, keepdims=True))
