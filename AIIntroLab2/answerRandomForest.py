...
def buildtrees(X, Y):
    ...
    trees = []
    n, d = X.shape
    sample_size = int(n * ratio_data)
    feat_size = max(int(d * ratio_feat), 1)
    for i in range(num_tree):
        sample_idx = np.random.choice(n, sample_size, replace=True)
        x_sub = X[sample_idx]
        y_sub = Y[sample_idx]
        feat_idx = np.random.choice(d, feat_size, replace=False)
        feat_idx = list(feat_idx)
        tree = buildTree(x_sub, y_sub, feat_idx, **hyperparams)
        trees.append(tree)
    return trees
