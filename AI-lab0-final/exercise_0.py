...
def q5():
    ...
    '''
    res = np.random.uniform(-1, 1, (1000000, 2))
    num = 0
    for i in range(1000000):
        if res[i][0]**2 + res[i][1]**2 <= 1:
            num += 1
    return 4 * num / 1000000
...
class Layout:
    ...
    def recover_layoutText(self):
        ...
        for wall in self.walls:
            layoutText[wall[0]][wall[1]] = '%'
        for food in self.foods:
            layoutText[food[0]][food[1]] = '.'
...
def q6(file_path):
    ...
    '''
    mp = Layout.build_layout(file_path)
    mp.recover_layoutText()
    return mp
