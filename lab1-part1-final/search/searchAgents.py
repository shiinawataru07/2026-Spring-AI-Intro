...
def cornersHeuristic(state, problem):
    ...
    if len(unvisited_corners) == 1:
        return util.manhattanDistance(node, unvisited_corners[0])
    dis1 = min(util.manhattanDistance(node, corner) for corner in unvisited_corners)
    dis2 = 0
    for i in range(len(unvisited_corners)):
        for j in range(i+1, len(unvisited_corners)):
            dis2 = max(dis2, util.manhattanDistance(unvisited_corners[i], unvisited_corners[j]))
    h_sum = dis1 + dis2
